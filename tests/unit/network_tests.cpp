...
void codec_tests(){
    ...
    Wire w=encode(p);Packet result;std::string error;check(decode(w.data(),w.size(),result,error),"roundtrip_decode");check(result.entity.id==19&&result.entity.dna==8.75f&&result.entity.vz==6&&result.request==991&&result.identity==p.identity,"roundtrip_fields");
    for(size_t n=0;n<packet_bytes;++n)check(!decode(w.data(),n,result,error),"every_truncation_rejected");check(!decode(w.data(),packet_bytes+1,result,error),"oversize_rejected");
    for(size_t offset:{size_t(0),size_t(4),size_t(8),size_t(12),size_t(16),size_t(20),size_t(24),size_t(31),size_t(383)}){Wire bad=w;bad[offset]=255;check(!decode(bad.data(),bad.size(),result,error),"header_enum_or_reserved_rejected");}
    p.entity.x=std::numeric_limits<float>::quiet_NaN();w=encode(p);check(!decode(w.data(),w.size(),result,error),"nan_rejected");p.entity.x=1;p.entity.owner=3;w=encode(p);check(!decode(w.data(),w.size(),result,error),"invalid_entity_owner");p.entity=entity(19,2);p.kind=Kind::action;p.direction=3;w=encode(p);check(decode(w.data(),w.size(),result,error),"native_direction3");p.direction=4;w=encode(p);check(!decode(w.data(),w.size(),result,error),"invalid_direction");p.direction=0;p.verb=Verb(4);w=encode(p);check(!decode(w.data(),w.size(),result,error),"retire_not_exposed");
    Digest d;check(parse_hex(hex(digest(97)),d)&&d==digest(97),"hex_roundtrip");check(!parse_hex(std::string(64,'z'),d)&&!parse_hex("abc",d),"invalid_hex");
}
struct Fixture {
    SessionConfig config=settings();Session session{config};uint64_t seq[16]{};
    std::vector<Dispatch> send(uint64_t c,Packet p){p.sequence=++seq[c];p.session=p.kind==Kind::hello?0:config.epoch;return session.receive(c,p);}
    std::vector<Dispatch> join(uint64_t c,Role role,uint64_t player=0){check(session.connect(c),"session_connect");Packet p;p.kind=Kind::hello;p.role=role;p.identity=config.identity;p.credential=role==Role::authority?config.authority:player==1?config.player1:config.player2;return send(c,p);}
    std::vector<Dispatch> baseline(uint64_t scene=1,uint64_t baseline_id=1){Packet p;p.scene=scene;p.baseline=baseline_id;p.kind=Kind::scene_begin;p.count=2;send(1,p);p.kind=Kind::entity;p.entity=entity(101,1);send(1,p);p.entity=entity(102,2);send(1,p);p.kind=Kind::scene_end;return send(1,p);}
};
...
void session_tests(){
    ...
    f.session.disconnect(2);auto reconnect=f.join(4,Role::player,1);uint64_t fresh=baseline_id(reconnect,4);check(reconnect.front().packet.player==1&&fresh>b2&&f.session.entity_count()==2,"reconnect_identity_fresh_baseline_no_duplicate");ack.baseline=b1;check(f.send(4,ack).empty(),"reconnect_old_ack_ignored");action.scene=1;action.baseline=fresh;action.entity=entity(101,1);check(action_refused(f.send(4,action),Error::not_ready),"obsolete_ack_does_not_restore_readiness");
    ...
    Fixture duplicate;duplicate.join(1,Role::authority);duplicate.join(2,Role::player,1);auto db=duplicate.baseline();Packet da;da.kind=Kind::baseline_ack;da.scene=1;da.baseline=baseline_id(db,2);duplicate.send(2,da);da.kind=Kind::action;da.entity=entity(101,1);da.request=10;check(duplicate.send(2,da).front().connection==1,"new_action_request");check(rejected(duplicate.send(2,da),Error::stale),"duplicate_action_request_not_executed");
    std::vector<Dispatch> last;for(size_t i=0;i<128;++i){da.request++;last=duplicate.send(2,da);}check(rejected(last,Error::capacity),"per_player_action_rate_bounded");
    Fixture rebase;rebase.join(1,Role::authority);rebase.join(2,Role::player,1);auto rb=rebase.baseline();uint64_t rb_old=baseline_id(rb,2);
    Packet old_ack;old_ack.kind=Kind::baseline_ack;old_ack.scene=1;old_ack.baseline=rb_old;rebase.send(2,old_ack);
    Packet begin;begin.kind=Kind::scene_begin;begin.scene=2;begin.baseline=2;begin.count=2;rebase.send(1,begin);
    Packet late;late.kind=Kind::action;late.scene=1;late.baseline=rb_old;late.request=900;late.entity=entity(101,1);late.player=999;
    auto refused=rebase.send(2,late);check(action_refused(refused,Error::not_ready)&&refused[0].packet.request==900&&refused[0].packet.player==1&&refused[0].packet.scene==1&&refused[0].packet.baseline==rb_old,"in_flight_action_during_rebase_correlated_refusal");
    check(rebase.send(2,old_ack).empty(),"in_flight_ack_during_rebase_ignored");
    begin.kind=Kind::entity;begin.entity=entity(101,1);rebase.send(1,begin);begin.entity=entity(102,2);rebase.send(1,begin);begin.kind=Kind::scene_end;auto committed=rebase.send(1,begin);uint64_t rb_new=baseline_id(committed,2);check(rb_new>rb_old,"rebase_fresh_baseline_after_in_flight_action");
    check(rebase.send(2,old_ack).empty(),"obsolete_ack_after_rebase_ignored");late.scene=2;late.baseline=rb_new;late.request=901;check(action_refused(rebase.send(2,late),Error::not_ready),"rebase_requires_fresh_ack");old_ack.scene=2;old_ack.baseline=rb_new;rebase.send(2,old_ack);late.request=902;auto good=rebase.send(2,late);check(good.size()==1&&good[0].connection==1&&good[0].packet.kind==Kind::action,"fresh_ack_restores_action_routing_after_rebase");
}
bool await_packet(Peer& peer,Kind kind,Packet& packet,std::string& error,unsigned milliseconds=5000){auto end=std::chrono::steady_clock::now()+std::chrono::milliseconds(milliseconds);Event e;while(std::chrono::steady_clock::now()<end){while(peer.poll(e)){if(e.kind==EventKind::packet){if(e.packet.kind==kind){packet=e.packet;return true;}if(e.packet.kind==Kind::reject){error=error_name(e.packet.error);return false;}}if(e.kind==EventKind::disconnected){error=e.detail;return false;}}std::this_thread::sleep_for(std::chrono::milliseconds(1));}error="await_packet_timeout";return false;}
...
void tls_tests(){
    ...
    try {
        ...
        Peer authority,p1,p2;check(authority.start(aconfig,error),"authority_network_start");check(p1.start(pconfig,error),"player1_network_start");check(p2.start(qconfig,error),"player2_network_start");Packet ap,pp,qp;require_packet(authority,Kind::welcome,ap,"authority_tls_welcome");require_packet(p1,Kind::welcome,pp,"player1_tls_welcome");require_packet(p2,Kind::welcome,qp,"player2_tls_welcome");check(pp.player==1&&qp.player==2&&pp.session==qp.session&&ap.session==pp.session,"real_tls_shared_session_distinct_players");
        Packet b;b.kind=Kind::scene_begin;b.scene=1;b.baseline=1;b.count=static_cast<uint32_t>(max_entities);check(authority.send(b),"send_baseline_begin");b.kind=Kind::entity;b.entity=entity(101,1);authority.send(b);b.entity=entity(102,2);authority.send(b);for(size_t i=2;i<max_entities;++i){b.entity=entity(1000+i,0);check(authority.send(b),"maximum_scene_entity_enqueued");}b.kind=Kind::scene_end;authority.send(b);require_packet(p1,Kind::scene_end,pp,"p1_actual_tls_baseline");require_packet(p2,Kind::scene_end,qp,"p2_actual_tls_baseline");check(pp.baseline!=qp.baseline&&pp.count==max_entities&&qp.count==max_entities,"real_maximum_scene_independent_baselines");Packet ack;ack.kind=Kind::baseline_ack;ack.scene=1;ack.baseline=pp.baseline;p1.send(ack);Packet act=ack;act.kind=Kind::action;act.entity=entity(101,1);act.verb=Verb::jump;act.player=999;p1.send(act);require_packet(authority,Kind::action,ap,"actual_action_to_authority");check(ap.player==1&&ap.entity.owner==1&&ap.baseline==1&&ap.request!=0,"actual_authenticated_action_owner");
        act.entity.id=102;p1.send(act);require_packet(p1,Kind::reject,ap,"actual_wrong_owner_rejection");check(ap.error==Error::ownership,"actual_wrong_owner_reason");
        auto old_baseline=pp.baseline;p1.stop();Peer resumed;check(resumed.start(pconfig,error),"reconnect_start");require_packet(resumed,Kind::welcome,pp,"reconnect_welcome");check(pp.player==1,"reconnect_same_player");require_packet(resumed,Kind::scene_end,pp,"reconnect_fresh_baseline");check(pp.baseline>old_baseline,"reconnect_fresh_baseline_id");
