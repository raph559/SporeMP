...
Inspected [Home at 1280×820](home-1280.png) and [Join at 1060×700](join-1060.png): original artwork and prominent Play stay intact; Join is a separate green card; the endpoint is legible, invitation text is masked, status/hints fit, and release cards remain accessible by scrolling. The password field clips its masked characters within the input as intended. Subsequent final [Connected](final-connected-1060.png) was inspected after aligning the sidebar/readiness strip and primary button with the active session. [Final Home](final-home-1280.png), [final Join](final-join-1060.png) and [final Rejoin](final-rejoin-1060.png) are retained from the same passing actual WPF fixture runner. Fixture protocol outputs are explicitly not original-game evidence. The earlier preview/provenance is preserved separately from the final source freeze in `launcher-provenance-final.json`.
