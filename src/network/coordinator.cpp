...
struct Coordinator::Impl {
    ...
    void dispatch(const std::vector<Dispatch>& messages){for(const auto& d:messages){auto c=channels.find(d.connection);if(c==channels.end())continue;const bool queued=c->second->send(d.packet);
        // Record action/content decisions, not a second copy of every scene packet.
        // Queueing is not a native application or a delivery acknowledgment.
        if((d.packet.kind==Kind::action||d.packet.kind==Kind::action_result||d.packet.kind==Kind::reject||d.packet.kind==Kind::content_response)&&events.size()<queue_capacity)
            events.push_back({d.connection,{EventKind::packet,d.packet,queued?"dispatch_queued":"dispatch_queue_full"},true});
        if(d.close){/* Rejection is delivered before close; application deletes after a short flush below. */closing[d.connection]=GetTickCount64()+100;}}}
    std::map<uint64_t,uint64_t> closing;
};
Coordinator::Coordinator():impl_(std::make_unique<Impl>()){}
Coordinator::~Coordinator(){stop();}
bool Coordinator::start(CoordinatorConfig config,std::string& e){auto& x=*impl_;if(x.session){e="coordinator_already_started";return false;}if(!valid_world_identity(config.session.identity.world)){e="missing_world_identity";return false;}if(!detail::initialize_sockets(e)||!x.identity.create(e))return false;Digest zero{};for(auto secret:{&config.session.authority,&config.session.player1,&config.session.player2})if(*secret==zero&&!detail::random_bytes(secret->data(),secret->size())){e="credential_random_failed";return false;}if(config.session.authority==config.session.player1||config.session.authority==config.session.player2||config.session.player1==config.session.player2){e="credentials_must_be_distinct";return false;}if(!detail::random_bytes(&config.session.epoch,sizeof(config.session.epoch))||!config.session.epoch){e="session_random_failed";return false;}x.listener=detail::listen_socket(config.bind_host,config.port,config.port,e);if(x.listener==INVALID_SOCKET)return false;x.config=config;x.session=std::make_unique<Session>(config.session);return true;}
