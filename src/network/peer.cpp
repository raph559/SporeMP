...
bool read_peer_config(const std::wstring& path,PeerConfig& result,std::string& error){
    ...
    const char* required[]{"schema","host","port","certificate_sha256","credential","role","build_sha256","executable_sha256","content_sha256","fixture_sha256"};if(values.size()!=10){error="unexpected_config_fields";return false;}for(auto key:required)if(!values.count(key)){error="missing_config_field";return false;}
    PeerConfig c;if(values["schema"]!="1"||(values["role"]!="player"&&values["role"]!="authority")){error="unsupported_config_schema_or_role";return false;}
