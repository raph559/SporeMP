...
Session::Session(SessionConfig c):config_(std::move(c)) {}
...
std::vector<Dispatch> Session::disconnect(uint64_t id){std::vector<Dispatch> out;connections_.erase(id);if(authority_==id){authority_=0;complete_=false;building_=false;staging_.clear();for(auto& pair:connections_){pair.second.ready=false;pair.second.baseline=0;reject(out,pair.first,Error::authority_lost);}}return out;}
std::vector<Dispatch> Session::receive(uint64_t id,const Packet& m){
    ...
    if(!c.authenticated){
        ...
        if(!(m.identity==config_.identity)){reject(out,id,Error::incompatible,true);return out;}
        uint64_t player=0;
    ...
    if(c.authority){
        if(m.kind==Kind::scene_begin){
            ...
            building_=true;complete_=false;staging_.clear();staging_scene_=m.scene;staging_baseline_=m.baseline;expected_=m.count;
    ...
    auto refuse_action=[&](Error reason){Packet response;response.kind=Kind::action_result;response.error=reason;response.player=c.player;response.scene=m.scene;response.baseline=m.baseline;response.entity=m.entity;response.request=m.request?m.request:m.sequence;emit(out,id,response);};
    ...
    if(actor==entities_.end()||actor->second.owner!=c.player||actor->second.generation!=m.entity.generation||(m.target&&!entities_.count(m.target))){reject(out,id,Error::ownership);return out;}
    Packet action=m;action.player=c.player;action.entity.owner=c.player;action.baseline=source_baseline_;if(!action.request)action.request=m.sequence;if(action.request<=c.last_request){reject(out,id,Error::stale);return out;}c.last_request=action.request;emit(out,authority_,action);return out;
}
}
