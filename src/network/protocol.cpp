...
bool Identity::operator==(const Identity& x) const noexcept { return build==x.build && executable==x.executable && content==x.content && fixture==x.fixture; }
namespace {
...
bool valid_entity(const Entity& e) noexcept {
    if(!e.id||!e.generation||e.owner>2)return false;
    ...
    return e.health>=0&&e.energy>=0&&e.hunger>=0&&e.dna>=0;
}
bool decode(const uint8_t* w,size_t n,Packet& out,std::string& error) {
    ...
    if(k<1||k>uint32_t(Kind::ping)||r<1||r>2||err>uint32_t(Error::queue_full)||m.count>max_entities){error="invalid_enum_or_count";return false;}
    ...
    if(m.kind==Kind::action){auto v=uint32_t(m.verb);if(v>6||v==4||m.direction<-1||m.direction>3||!e.id||!e.generation){error="invalid_action";return false;}}
